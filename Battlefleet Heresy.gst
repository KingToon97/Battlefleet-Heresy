...
<gameSystem xmlns="http://www.battlescribe.net/schema/gameSystemSchema" id="86405571-5289-597b-b308-a9a2d68c00b9" name="Battlefleet Heresy" revision="14" battleScribeVersion="2.03" authorName="Battlefleet Heresy Community Data" authorContact="" authorUrl="">
 <costTypes>
 ...
 <sharedRules>
  ...
  <rule id="a78bf71f-7d79-5b3d-a53e-22f6f9dd595d" name="Boarding Actions" hidden="false">
   <description>Crew are the hit points of boarding. An action begins and ends within one Ramming &amp; Boarding step. • Commit: declare the Crew committed and how they are delivered, then deduct them. A ship always keeps 1 Crew aboard. • Delivery cap: a ship may deliver no more than half its own Crew, rounding down, in one battle round, however many craft or pods it launches. A Hull Assault from base contact is exempt. • Fight: every fighter rolls his own D6 with his own bonus; bonuses never combine. Starting with the boarders, players alternate nominating duels. The higher total fells its opponent; ties are re-rolled until one is higher. • End: fight on until one side reaches 0 Crew or the boarders withdraw. Defenders reduced to 0 lose the ship — Capture or Scuttle. • Modifiers, applied to every die on that side: +1 boarders in the first round if delivered by craft, pod or teleport, never a Hull Assault · +1 boarders if the enemy ship is Crippled · +1 boarders if it is Engines Crippled · −1 defenders if the ship carries any Critical Damage. These are cumulative. • Two ships boarding one target resolve separately, in the attacker's chosen order; the defender commits Crew afresh to each from what remains.</description>
  </rule>
  ...
  <rule id="73f4d3d5-5d46-57ba-8784-a05013d88da4" name="Fleet Construction" hidden="false">
   <description>• Capital allowance: 1 Capital Ship per 4 individual Line Ships — 0–3 Line permit none, 4–7 permit 1, 8–11 permit 2, and so on. Count ships, not roster entries: a squadron of four Destroyers is four Line Ships. • Line Ships: Destroyer, Frigate, Light Cruiser, Strike Cruiser, Cruiser. • Capital Ships: Battlecruiser, Grand Cruiser, Battle Barge, Battleship, Gloriana, Abyss and the Phalanx. • No single ship may exceed half the fleet's points, upgrades included. • Squadrons: Destroyers and Frigates are taken singly, up to 3 to a squadron. • Minimum game size: Gloriana 2,000 points; Abyss and Phalanx 3,000.</description>
  </rule>
  <rule id="3b4ce7bc-d447-5c04-a590-91655478b604" name="Fleet Orders" hidden="false">
   <description>Fleet Orders are issued to the fleet, not to individual ships, and have no range. At the Muster step a fleet selects &lt;b&gt;1&lt;/b&gt; Fleet Order for the battle round; that order is then available to every eligible friendly ship on the table, however far from the Flagship it lies. A Named Character raises this to 2 and a Primarch to 3, and where a trait or ability says so, more. FLAGSHIP COMMAND • While your Flagship remains on the battlefield your fleet issues Fleet Orders normally. • If your Flagship is destroyed the fleet immediately loses access to Fleet Orders for the rest of the battle. • Orders already resolved this round are unaffected, and an order already issued and in effect is not cancelled. • A damaged, Crippled or shieldless Flagship commands normally — only her destruction ends it, unless a rule expressly says otherwise. • No other ship becomes the Flagship in her place. No ship is obliged to use the selected order. A ship suffers an order's penalties or restrictions only if it actually takes the benefit: if All Ahead Full is selected but a ship makes only its normal move, it fires normally. ALL AHEAD FULL (Movement) • Used: as a ship moves. • Effect: move +D6″. • Restriction: a ship that takes the extra movement may fire only Defence Batteries. EVADE (Movement, Reaction) • Used: when a ship is shot at or rammed, before any dice are rolled. • Effect: on 5+ the whole attack misses, or the ram fails and the rammer is moved 1″ past. • Modifiers: +1 Destroyer or Frigate; −1 Crippled, Adrift or Engines Crippled; never worse than −2. • Restriction: once per ship per battle round. A ship that held station this round may not Evade, and wrecks and Speed 0 installations never Evade. DAMAGE CONTROL (Command) • Used: in a ship's activation. • Effect: roll a D6; on 4+ choose one — put out a fire, repair one destroyed weapon system, or clear Engines Crippled. • Restriction: once per ship per battle round. The ship fires normally. RELOAD ORDNANCE (Shooting) • Used: as a ship fires. • Target: one weapon type aboard it. • Effect: remove one Reload token, freeing every weapon of that type aboard. • Restriction: a weapon type bearing no token cannot be chosen. RETURN FIRE (Shooting, Reaction) • Used: when a ship is attacked and survives. • Effect: fire back at −1 to hit with every weapon that bears on the attacker. • Restriction: not systems already fired this round, nor one-shot or reloading weapons. OVERWATCH (Shooting, Reaction) • Used: when a boarding craft or drop pod runs at a ship, for the rest of that battle round. • Effect: snap-fire Defence Batteries at that craft; destroyed on 5+. • Restriction: Defence Batteries only.</description>
  </rule>
  <rule id="8800bfa9-82a2-5c63-8e52-cef274c0e25c" name="Hit-and-Run" hidden="false">
   <description>Instead of a full boarding, commit 1 Crew — so never a Dreadnought, which is two. Roll D6 plus the raider's own bonus. On 4+ choose one: destroy one weapon system or Launch Bay; inflict 1 Critical Damage; cripple the enginarium (Engines Crippled); or sabotage the shields, reducing VS Rating by D3, or D6 against a Capital Ship, permanently. The raider returns. On a failure the raider is lost.</description>
  </rule>
  ...
  <rule id="84ae4fff-db0c-5d03-88d0-5a974e478c6a" name="Keyword: Caestus Assault Ram" hidden="false">
   <description>The Caestus Assault Ram is a boarding craft, not a gunship: bought into a Launch Bay for 60 pts, Armour 10, Speed 20″. It carries 4 Crew of boarders rather than 1, and its magna-melta bore opens the hull directly — boarders it delivers ignore the −1 for a Hull Assault and land as though delivered by craft. It may not be intercepted by Defence Batteries on the round it strikes.</description>
  </rule>
  <rule id="7c9d3b45-ec00-5fca-a050-b3dd28987708" name="Keyword: Critical Damage" hidden="false">
   <description>Fires, breaches and burning phosphex — one rule for all of them. A ship is either burning or it is not: &lt;b&gt;Critical Damage does not stack&lt;/b&gt;. A ship already carrying it that would take more simply carries on burning. At the end of each battle round a burning ship loses 1 Crew, with no roll. A successful Damage Control order puts the fire out. Mark it with a single token.</description>
  </rule>
  <rule id="72c83722-42a4-5b47-b361-df40e57ed6e2" name="Keyword: Destroyer (D)" hidden="false">
   <description>A Destroyer weapon is any weapon of AP D. It penetrates automatically — no roll, whatever the Armour Value — and never merely glances. Its Damage Table roll is made at +3, in place of any AP modifier, or at +2 against a Gloriana, the Abyss or the Phalanx. It is always at least Devastating (3), and its Strength never exceeds 12. • A Nova Cannon is stopped by a live shield: it collapses that shield and does nothing more. • A Vortex Torpedo is a torpedo and ignores shields entirely, striking the hull whether they hold or not. • The Destroyer hull class is unrelated. A Destroyer fires ordinary weapons at their printed AP.</description>
  </rule>
  <rule id="790e7556-1cf0-5ae8-94c9-0203fea91707" name="Keyword: Devastating (X)" hidden="false">
   <description>Each unsaved hit costs the target X Hull Points instead of 1, whether it glances or penetrates. A penetrating Devastating hit still rolls once on the Damage Table, keeping its AP modifier. Against active shields a Devastating hit still costs only 1 Void Shield Rating, however large its X.</description>
  </rule>
  <rule id="5cd591fe-92a6-5e71-b14b-a33270e7ce22" name="Keyword: Interceptor" hidden="false">
   <description>A Defence Battery may fire the moment an enemy squadron comes within 12″ as the enemy moves, before it attacks — but then does nothing else that battle round, Overwatch included.</description>
  </rule>
  ...
  <rule id="53724fbb-9c1d-536e-a6d9-551c39f36bcf" name="Keyword: Melta Mines" hidden="false">
   <description>The Falchion-class Frigate carries two Melta Mines; no other ship has them. • Laying: in the Movement step, after it has moved and instead of shooting, place a mine on a 32mm base touching the ship's base. One mine per battle round, Reload (2), and never within 6″ of an enemy ship. • Triggering: a mine detonates the moment any part of a ship's base passes over it. Frigates of either fleet pass safely, and aircraft ignore mines entirely. • Blast: every ship within 3″ — friend, foe, and the ship that triggered it — suffers one hit at Strength 9, AP 2, Armourbane, against the facing nearest the mine. Mines ignore Void Shields. Remove the marker once the blast is resolved.</description>
  </rule>
  ...
  <rule id="a46e50ef-8ede-570d-b475-c0d9d2c54d97" name="Keyword: Reload (X)" hidden="false">
   <description>Reload applies to a weapon TYPE aboard a ship, not to a single mount. When a ship fires a weapon that has Reload (X), place X Reload tokens against that weapon type: every weapon of that type aboard the ship is spent together and none of them may fire while a token remains. So a ship mounting four Twin Lance Turrets fires all four or none, and all four reload together. Flip the token at the end of each battle round — 2 becomes 1 — and remove it when it would reach 0. Reload (1) speaks every round and Reload (2) every second. The Reload Ordnance order removes one token early, freeing that whole weapon type to fire again.</description>
  </rule>
  ...
  <rule id="8e3796e2-b815-58c7-b6b4-33961c550a20" name="Keyword: Vortex Torpedo" hidden="false">
   <description>Vortex Torpedoes are carried by Gloriana-class vessels and the Furious Abyss alone. No other hull may mount them and they are not a purchasable upgrade. A Vortex Torpedo occupies one Prow mount and may be mounted nowhere else. Strength 12, AP D, 60″, One Shot, Devastating (3), Volatile. Like any torpedo it bypasses Void Shields entirely, resolving against the hull facing struck; unlike a Nova Cannon it is not stopped by a live envelope.</description>
  </rule>
  ...
  <rule id="beb38486-02f1-5ef4-abdb-6b853cbd1309" name="Penetration" hidden="false">
   <description>Roll a D6 per hit and add the weapon's Strength. Beat the target's Armour Value for a Penetrating Hit; equal it for a Glancing Hit; below it, nothing. AP steps the weapon up the scale: AP 3 is the baseline, AP 2 adds +1 to the Damage Table, AP 1 adds +2, and AP D penetrates automatically. Where a rule grants +1 AP, a weapon at AP 3 becomes AP 2, AP 2 becomes AP 1, and a weapon already at AP 1 or AP D instead gains Armourbane for that attack. Void Shields are tested before armour: a shot that penetrates the shield's AV collapses it and is stopped there.</description>
  </rule>
  ...
  <rule id="74145fd7-84df-5d17-8261-bab70c919268" name="Ship Damage Table" hidden="false">
   <description>Roll a D6 when a Penetrating Hit is scored. Modifiers: AP 2 +1 • AP 1 +2 • Destroyer +3 in place of any AP modifier, +2 against a Gloriana, the Abyss or the Phalanx • Ramming +1 • Battleship target −1. A modified 7+ counts as Reactor Struck; below 1 counts as 1, and a 1 does nothing. 1 HELD • Nothing further happens. 2 BLAZE • Immediate: 1 Critical Damage. • Ongoing: the ship loses 1 Crew at the end of each battle round while it burns. • Repair: Damage Control clears it. 3 HULL BREACH • Immediate: the ship loses 1 Crew. • Ongoing: none. • Repair: none. 4 MAGAZINE EXPLOSION • Immediate: 1 Hull Point, 2 if the ship mounts Triple Lance Turrets, plus 1 Critical Damage, and one random Port, Starboard or Dorsal weapon system destroyed. • Ongoing: the destroyed system cannot fire. • Repair: Damage Control may repair one destroyed system. • Note: Blast-Shielded Magazines reduces the Damage Control suffered by 1, to a minimum of 0. 5 ENGINES CRIPPLED • Immediate: immobile, and Snap Shots only. • Ongoing: no holding-station broadside bonus, and −1 on Evade. It may still take Fleet Orders. • Second result: the ship is Adrift — 2D6″ straight ahead, no turning. • Repair: Damage Control clears it; Adrift becomes Engines Crippled. 6 REACTOR STRUCK • Immediate: a Destroyer, Frigate, Light Cruiser or Strike Cruiser suffers a Catastrophic Explosion at once. A Cruiser, Battlecruiser, Grand Cruiser, Battle Barge or Battleship loses D6 Hull Points; a Gloriana, the Abyss or the Phalanx loses D3. If that loss takes the ship to 0 Hull Points it suffers a Catastrophic Explosion automatically, with no roll and no chance of becoming a wreck; otherwise nothing further happens. • Ongoing and Repair: none.</description>
  </rule>
  <rule id="5d03420a-e902-5c74-b9b9-03adfd1d514d" name="Ship Type Abilities" hidden="false">
   <description>Every ship carries a Ship Type keyword beside its category: a Lunar is Line, Cruiser; a Mars is Capital, Battlecruiser. The type grants an ability shared by every class of that type. • Destroyer — Wolf Pack: squadron mates within 6″ combine their Torpedo Tubes on one target at +1 penetration. Torpedoes only. • Frigate — Screen: enemies suffer −1 BS against a friendly Capital Ship within 3″. Never stacks. • Light Cruiser — Steadfast: +1 to Evade rolls. • Strike Cruiser — Engage the Enemy: re-roll failed boarding damage rolls. • Cruiser — Enduring Hull: Damage Control succeeds on 3+. • Battlecruiser — Battleline Momentum: fires normally under All Ahead Full. • Grand Cruiser — Fire Control Coordination: splits fire without the −1 BS penalty. • Battle Barge — Relentless Assault: Drop Pod Launchers are not One Shot and reload normally. • Battleship — Bulwark of the Line: −1 to Damage Table rolls against this ship. A type grants its ability and nothing else — never another class's weapons or upgrades.</description>
  </rule>
  ...
  <rule id="5a6291fd-05f0-5e71-9f5f-e9f0afbad8aa" name="State: Engaged" hidden="false">
   <description>Two aircraft squadrons within 1″. Neither may act against ships, and no ship may fire on either — Defence Batteries, Interceptor and Overwatch included. Resolved at the end of the battle round.</description>
  </rule>
  ...
  <rule id="a1ec0cc4-2326-5b5f-ad5e-8e0c6d306590" name="Stern Strike" hidden="false">
   <description>When a Strength 9+ weapon rolls a natural 6 on its penetration die against a target's Stern and beats the Stern AV, the hit also inflicts Engines Crippled at once. It does not stack with an Engines Crippled already rolled, and while shields hold the hit must bring them down first.</description>
  </rule>
  ...
  <rule id="b00e13cb-eb88-50c1-b468-fd81d592e0c0" name="Universal Ship Upgrades" hidden="false">
   <description>Open to every ship below Gloriana class, whatever its Legion or role. Glorianas, the Abyss and the Phalanx may not take them. Each may be bought once per ship, in any combination. Each costs a share of the ship's own points, rounded to the nearest 5: • Veteran Crew 30% — +1 BS, to a maximum of BS 5. • Servitor Autoloaders 30% — every weapon aboard reduces its Reload by 1, to Reload (0). • Adamantium Belt Armour 20% — +1 Side Armour, to a maximum of AV 14, and −2″ Speed to a minimum of 4″. • Plasma Drive Overhaul 12% — +2″ Speed. • Emergency Shield Capacitors 10% — re-roll failed Void Shield regeneration rolls. • Phosphex Shell Magazines 8% — a penetrating hit from any Heavy Macro-Cannon Battery aboard inflicts 1 Critical Damage. • Blast-Shielded Magazines 8% — a Magazine Explosion costs 1 less Damage Control, to a minimum of 0.</description>
  </rule>
  <rule id="0591ff9a-8270-5c08-8d3e-0615e5b1fc75" name="Veteran Crew and Accuracy" hidden="false">
   <description>Any ship may take a Veteran Crew once: +1 Ballistic Skill for the battle. It costs 30% of the ship's own points value, rounded to the nearest 5. No ship exceeds BS 5. A ship takes at most one +1 BS from position or command — the holding-station broadside bonus and accuracy auras never stack with one another — and a Veteran Crew's +1 is a paid upgrade that stacks on top of one such bonus. For commanders, see Characters &amp; Primarchs.</description>
  </rule>
